...
type Config struct {
	...
	// PromptType will be used fot the prompt parameter (when offline_access promptType should be "consent")
...
func (c *HSDPConnector) LoginURL(s connector.Scopes, callbackURL, state string) (string, []byte, error) {
	...
	if c.isSAML() {
		...
		if err != nil {
			return "", nil, fmt.Errorf("invalid SAML2 login URL: %w", err)
		}
		values = u.Query()
